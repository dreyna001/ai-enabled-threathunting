...
These commands cannot qualify provider credentials by themselves. The live known-answer runner additionally requires `THREAT_HUNTING_KNOWN_ANSWER_FIXTURE_ID`, `THREAT_HUNTING_KNOWN_ANSWER_LIVE_ADAPTER=module:function`, and `THREAT_HUNTING_KNOWN_ANSWER_BINDINGS_FILE` pointing to a protected private bindings file outside version control. That file must map all twelve public scenario IDs to fixture identity values and execution/export sources; it is not committed. Without bindings, an application-owned fixture workflow, or an actually implemented deterministic fault injector for timeout/model-repair/restart-recovery/hard-budget/cancellation scenarios, live mode exits with a blocker and does not claim qualification. Naming an injector in the bindings file does not implement it. A Docker engine outage blocks the container checks; Splunk URL/TLS/token failure blocks discovery and execution; a missing or unqualified model secret blocks plan generation or synthesis. Record the blocker and use Path A for deterministic workflow verification.

The live callback receives only `scenario_id`, `fault_mode`, `execution_source`, and `export_source`. Evaluator categories, evidence mappings, expected counts, answer limitations, and answer keys remain runner-side. Locally exercise the five workflow fault paths with `.venv/bin/python -m pytest tests/known_answer/test_fault_qualification.py -q`; this uses test adapters and does not unlock or replace the deployed twelve-scenario live matrix.

## Model output evaluation

After deploying prompt contract `1.34`, generate and approve a fresh plan. Update API and worker images together and set the runtime `image_version` to the immutable release tag. OpenAI-compatible endpoints must accept strict `json_schema` response formats; Bedrock models must support Converse `outputConfig.textFormat`. There is no automatic downgrade to JSON mode. These shapes follow the [OpenAI structured-output contract](https://developers.openai.com/api/docs/guides/structured-outputs) and [Bedrock structured-output contract](https://docs.aws.amazon.com/bedrock/latest/userguide/structured-output.html).
...
The local prompt-1.33 candidate also requires every approved question to have a concise summary plus supported findings or explicit limitations. The application assigns question/finding identities. All generated findings stay in retained hunt results. The report draft defaults to ten selected finding details across questions, favoring hunt leads within each question, while preserving every question summary and full finding references; analysts may include more within the existing 1 MiB report input limit. This default does not restrict investigation memory, evidence retrieval, or model output. PDF/HTML question answers print summaries, limitations, and reference counts. The app expands complete findings for each question and links to its own retained results, not the Splunk search UI. No new customer configuration is required. Missing support must remain explicit, and valid answer slots do not establish complete or correct analysis. This candidate remains undeployed.

Prompt 1.30 retains time-spread evidence selection and derives requested scoped inventories with explicit missing/multivalue counts. It adds bounded retained-page requests by completed query, exact field values and optional UTC time windows. A question either returns a final answer or requests up to three pages, each at most 500 representation groups. Identical raw representations retain all citation origins; differing fields remain separate. Completed answers/pages are checkpointed, repeated requests stop retrieval, and original query coverage remains distinct from local page/sample coverage. Before reserving a retrieval round, the application checks whether the complete retained evidence fits one final request under existing row, character and remaining-token limits. If it fits, final synthesis receives all those records and the remaining time. Otherwise bounded retrieval retains its final-call/token reservation. A failed submitted final call is not restarted as retrieval. Native process/session identifiers may be quoted as observed facts; only citation fields use request-local labels. Complete-request preflight enforces `context_characters` and uses UTF-8 bytes plus framing as a conservative input-token estimate. Decoded model text, schemas and tools count toward this estimate; the extra JSON escaping used to transport those strings does not. Synthesis adjusts the evidence sample while rebuilding coverage. Repairs retain their original evidence mapping and must also fit. Budget exhaustion leaves explicit unanswered slots. This candidate remains undeployed and unqualified; see the [resume checkpoint](resume-checkpoint.md) for measured fitting limits and remaining checks.

Every supplied raw advisory-match review group now requires a per-question disposition linking responsive findings or naming unanswered support. Groups preserve all evidence origins and use equal observed host/process GUID/user/session literals when available; missing or ambiguous identity remains separate. These are review groups, not asserted unique process lifetimes. Positive findings linked to a lead must cite a supplied record for that lead. The existing repair handles missing/duplicate lead entries and invalid finding positions. Native identity fields and explicit gaps are computed or preserved by the application and shown in the app and reports; report edits cannot drop or reassign this coverage. Omitted retained leads and exhausted budgets remain explicitly unanswered. Mechanical coverage does not prove semantic completeness or factual correctness.

Prompt 1.30 moves inventory quantities into application-owned question tables. The model selects up to three completed-query scopes with optional exact typed filters and approved UTC bounds; it cannot supply count fields. Counts are calculated over the complete matching retained raw subset after selection, without materializing copied raw pages. Missing fields, multivalue rows, unknown times and incomplete retrieval remain explicit. Final model input no longer contains whole-query count tables for prose transcription. The UI and PDF/HTML show the computed quantities and exact scope; report edits preserve both values and scalar types. This does not validate arbitrary model prose or prove that its chosen scope fully answers the question.

The prompt-1.33 candidate selects each lead reference from its earliest supplied advisory observation with a known timestamp. Its compact reference index matches literal process or host/user/session filters and groups supplied records before, at, or after that lead time, then by observed action. An unknown anchor or record timestamp gives an unknown time relationship. Application-calculated first/last times bound the known observations within each action and period; references are ordered by event time with unknown times last. The lead need not be a process start, and these bounds do not establish a process lifetime or continuous activity. The index preserves original citation origins and positive observations from truncated queries. It is navigation, not a unique-event count or proof of identity, causation, absence, or complete coverage. Native source fields still establish individual claims; no fixture answer keys, extra provider calls, or new settings are introduced.

V38 (prompt 1.33) clarifies synthesis alongside the application-owned timelines and scoped counts. Every factual premise in inference or limitations requires the finding's own citations, including both relationship endpoints. No-match conclusions require separate successful complete-query findings; field availability and retrieval completeness retain their actual source/query scope. The conflicting instruction to copy inventory quantities into prose is removed. Schema shapes, validators, model/settings and frozen evidence are unchanged. Existing schema and reference validation does not establish prose truth. The candidate remains undeployed and requires analytical qualification; see the resume checkpoint for current evidence.

Prompt 1.31 shares lead and general evidence citation choices in the native response schema. A full 500-record batch with many distinct leads remains within the documented provider enum limit; lead-only fields still reject nonlead records and all citation fields reject unknown labels. This changes schema capacity, not analytical coverage.
...
The local SPL policy 1.5 candidate rejects undiscovered fields referenced in actual search predicates, eval/where expressions, aggregate inputs, projections, sorting, deduplication and regex filters. Input metadata cannot hide a field reference. Static eval assignments are checked left-to-right; rename and aggregate aliases become available after their command. `requested_fields` continues to list discovered input fields, not generated outputs. Quoted string values are not treated as source fields. Dynamic eval field names and unsupported field syntax are rejected explicitly; wildcard renames must have matching wildcard counts, bounded to 32 per name. Versioned eval/where and aggregate function allowlists are enforced and supplied to query generation. `lookup`, `searchmatch`, and customer-defined functions are rejected with `function_not_allowed`; string literals cannot conceal an executable search or an extra lookup source. This check does not add new SPL commands. Roll out API and worker together and obtain a fresh plan approval because the policy version is included in the execution binding and query cache key.
...
- Safe validation diagnostics: `usage.model_output_checks[].validation_error_code` retains a fixed schema, JSON, reference-label, citation-relationship, query-coverage, or structure code even when a subsequent repair cannot start. It contains no model text or validation input.
- Per-call sizing and usage: `usage.model_output_checks[]` records `context_characters` and `estimated_input_tokens` before each submitted call, plus provider-reported `input_tokens` and `output_tokens` when a response arrives, including invalid outputs and repairs. The estimate is distinct from actual usage. Missing historical or failed-transport usage stays `null`; do not reconstruct pre-synthesis usage by subtracting a different diagnostic from an old hunt total.
...
Before changing a deployed configuration, finish active jobs, preserve the database and prior image/configuration, and update both API and worker. Existing approval snapshots remain immutable; new settings require fresh discovery/approval. The standard YAML settings use `hard_completion_minutes: 20` and `query_count: 50`; the temporary local evaluation below intentionally differs.

On September 10, the user requested more execution headroom while correcting analytical behavior. This desktop deployment now selects `runtime/config/local-20260910-evaluation-limits.yml` through `.env`. API and worker were recreated together using their existing v18 image and unchanged volumes. The temporary allowances are 60 minutes, 16 cycles, 100 searches, 48 model calls, 5,000,000 total input tokens, 512,000 total output tokens, 64,000 output tokens per call, and 600 seconds per model call. The query/adaptive cutoff is minute 48, outstanding searches stop by minute 50, and ten minutes remain reserved for synthesis. Representative and targeted model batches are both 500 records; context allowance is 800,000 characters. Existing deployed v18 does not enforce that character setting; the current-source candidate does. Provider/schema limits and existing retention, concurrency, cancellation, ownership and approval rules still apply. These are evaluation allowances, not a claim that the app meets the standard production budgets.

Use this exact local profile for new diagnostics and record actual consumption. Historical replay scripts that explicitly name the prior v18 configuration retain their original settings; do not silently rerun them as the current evaluation. The old configuration remains unchanged at `runtime/config/local-20260909-quality-v18-231548.yml`. Rollback restores `.env` from `runtime/qualification/quality-fixes-20260910/v34-evaluation-limits/rollback/previous.env` and recreates API and worker with that configuration, after checking for active work. No model or fixture change accompanies this temporary increase. Verification and database fingerprints are in `v34-evaluation-limits/verification.json` under that qualification directory.
