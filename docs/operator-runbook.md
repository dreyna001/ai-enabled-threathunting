...
The prompt-1.31 candidate selects each lead reference from its earliest supplied advisory observation with a known timestamp. It also supplies a compact reference index by literal process or host/user/session filters and observed action, ordered within each action by event time with unknown times last. The index contains only supplied raw representation groups, preserves original citation origins, and includes positive observations from truncated queries. It is navigation, not a unique-event count or proof of identity, causation, absence, or complete coverage. Native source fields still establish claims; no fixture answer keys, extra provider calls, or new settings are introduced.
