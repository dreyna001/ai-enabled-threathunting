...
When all retained evidence fits a final request under current row, character and remaining-token limits, synthesis uses that complete request without reserving unnecessary retrieval capacity. Preflight failure before submission may select the bounded retrieval path; a submitted final call is never replayed as retrieval. Citation-field labels must not be confused with native process/session identifiers requested as source facts. This does not establish complete analytical coverage; real-provider replay and claim review remain required.
...
For application-materialized question lead groups, results also derive `lead_activity`: lead references and anchors, shared exact-literal process/session scopes, original evidence references, temporal partitions, action bounds, typed field counts and per-query retrieval status. A missing process GUID can still permit a known host/user/session selection; ambiguous native identity prevents unsupported correlation. A lead anchor is the earliest known retained matching observation, not necessarily a process start. Unknown timestamps or anchors cannot establish a relative period. Usable observations from failed or partial retrieval remain included with their own status. Shared scopes are indexed once rather than copying raw evidence per lead or question. The answer view can select a lead and recorded relationship, inspect all scoped records and navigate back to their original sources without model-selected findings or inventories. Counts describe retained representations/literal values, not unique events, proven process instances or affected entities; overlapping scope counts must not be added, and an empty selection does not prove source absence.

New concise reports include a protected `observed_activity` summary for up to ten lead groups and twenty action groups per selected scope, explicitly reporting totals and retaining the full view in hunt results. Active PDF output includes the summary. Report edits cannot change the derived facts or remove an existing summary; historical reports without the section remain unchanged. The five-lead frozen qualification example fits without omitting a lead or action group. This is factual presentation and does not establish correctness of model prose, full-hunt qualification or independent analyst acceptance.
