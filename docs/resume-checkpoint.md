...
- The saved v22 patch and all ten base/candidate file hashes were verified before applying the patch here. Frozen fixture hashes still match. Prompt binding is now **1.27**, policy **1.5**. The candidate is not deployed or analytically qualified.
...
- Latest pre-publication verification: **642 backend tests passed, no skips**, two existing dependency warnings; mypy passed on **55 source files**. See `backend-tests-coverage.log` and `mypy-coverage-final.log`. The preceding full run had 629 passes and one new cancellation-test expectation failure: cancellation correctly revoked the worker lease. The corrected test verifies rejection, unchanged checkpointed findings and zero additional model calls.
- Reconciled older fixes for effective Splunk index permissions and source-query citation validation. Discovery now filters explicit/inherited allows and denies, fails closed on unavailable permissions, and preserves differing field observations with a limitation. The generic citation validator resolves source-query lineage and rejects failed or incomplete query coverage for negative claims. The active synthesis path now excludes failed, truncated, partial or incompletely retrieved query results from negative-finding choices, rejects invalid responses through the existing single repair, and enforces the boundary again before persistence. Positive observations can still cite retained rows from truncated queries. The workflow regression preserves retained evidence and produces explicit limitations after repairing an invalid negative claim.
...
- Published checkpoints `66c40d4` and `b8d39b1` to remote `main`, now the GitHub default branch and this checkout's upstream. GitHub Actions initially exposed `ModuleNotFoundError: scripts` under the pytest console entry point. The same failure was reproduced locally and fixed by declaring the repository import path in pytest configuration. All **630 tests** then passed using that entry point, the lock check passed, and [GitHub Actions run 34506084289](https://github.com/dreyna001/ai-enabled-threathunting/actions/runs/34506084289) passed backend tests/types/image build and frontend tests/build at `b8d39b1`.
- Follow-up environment identification corrected the Splunk mismatch above: the actual configured instance is **`threat-hunting-splunk-test-splunk-1` on 18089**, using `threat-hunting-splunk-test-etc` and `threat-hunting-splunk-test-var`. It had the same missing WSL bind-mount failure as the application containers. It was recreated from the existing pinned Compose definition with those same volumes and existing accepted startup settings; its administrative secret mount now comes from this desktop workspace. The recovery definition is `runtime/config/restore-existing-splunk.yml` (ignored, local only). Startup completed through the image's existing retry logic. The container is healthy, and the saved application token passes metadata-only discovery through the current candidate: nine permitted indexes, no discovery errors. See `restored-live-metadata-discovery.json`. No search, fixture reload or hunt was submitted during this recovery. The different `threat-hunting-splunk-live-20260906` instance on 28089 was accidentally started during the earlier restoration and was stopped with its volumes preserved. Do not point the app at it or replace/reload either dataset. The application database still has 42 hunts, zero active jobs and unchanged fingerprints.


- Prompt **1.24** also validates retained lookup windows against the approved plan inside the model contract, allowing one repair with the original evidence instead of failing the hunt after a structurally valid request. Blank filter names are rejected; filter fields not observed in retained raw results are reported explicitly, so zero local matches do not establish source absence. Six negative-coverage regressions and four lookup-boundary regressions failed before their fixes. Final relevant checks passed 99 unit tests, 26 workflow integration tests, the full 642-test backend suite and typing on 55 source files. This is deterministic correctness evidence, not analytical acceptance.

- Prompt **1.25** corrects request sizing to count decoded model text, schemas and tools without counting the extra escaping used for HTTP transport. All 447 frozen trial-13 representations now fit with an unused budget, compared with 411 immediately before this correction. Under the recorded historical **total-usage** stress case, 89 representations fit, compared with 74 before; the half-budget retrieval allocation still rejects its minimum request before a provider call. Historical totals include the old synthesis and cannot establish exact pre-synthesis capacity. This remains a conservative UTF-8-byte estimate, not measured provider tokens. Each submitted call now records request characters and its input estimate; returned responses also record actual input/output tokens, including invalid outputs and repairs. Missing older or failed-transport usage remains unknown. The regression reproductions failed before the correction; 75 focused tests, all **645 backend tests** (no skips, two existing dependency warnings), and mypy on 55 source files passed. See `runtime/qualification/quality-fixes-20260910/v24-context/` for before/after fitting evidence and verification logs. No provider calls, searches, fixture changes, deployments or new hunts were needed for this slice.
- Published prompt-1.24 checkpoint `e38740e` passed both backend and frontend jobs in [GitHub Actions run 34508182631](https://github.com/dreyna001/ai-enabled-threathunting/actions/runs/34508182631). The workflow was introduced with the September 9 consolidated local changes and runs on pushes and pull requests. Remote automated code checks do not satisfy the pending live hunt and independent analyst gates.

- Prompt **1.26** / SPL policy **1.5** reconciles the remaining useful older field-validation behavior in the current architecture. Actual query predicates, expressions, aggregate inputs, projections, sort/dedup fields and regex fields are checked independently of declared input metadata. Static eval fields are introduced left-to-right, and renamed/aggregate fields only after definition. Quoted literals and supported wildcard aliases remain valid. Versioned eval/where and aggregate function allowlists close string-embedded search and external lookup paths and are supplied to query generation. Dynamic eval names and unsupported field syntax fail closed. Nineteen original field regressions and four function-boundary regressions failed before their respective fixes; the final full backend suite passed **701 tests, no skips**, with two existing dependency warnings, and mypy passed on 55 source files. All 12 frozen trial-13 queries still validate; rejected source/function cases submit no search. See `runtime/qualification/quality-fixes-20260910/v25-field-policy/`. The older checkout still needs archival/retirement after final inventory verification; its full architecture was not merged.
- Prepared a separately labeled replay through `WorkflowService.execute_job` using an isolated SQLite copy of trial-13 evidence and no usable Splunk transport. The dry rehearsal completed four answers and one retained-page request across two fake calls. Its first setup attempt failed on an incorrect harness attribute name, before any model call; that harness-only mistake was corrected. The original evidence and query arrays remain unchanged. A fresh synthesis-only budget is explicit and excludes historical investigation costs; it cannot establish full-hunt resource qualification. Live replay and claim review remain pending. The saved local runtime and v21 verification artifacts both report **OpenAI gpt-5.6-terra / medium**; the shipped template's gpt-5.1 / none defaults and older narrative notes must not override the actual recorded configuration. Replay preparation is under `runtime/qualification/quality-fixes-20260910/v26-replay/`.

- Published field/function checkpoint `96e00cc` passed both jobs in [GitHub Actions run 34512074287](https://github.com/dreyna001/ai-enabled-threathunting/actions/runs/34512074287).
- The current production-loop replay completed at prompt **1.26**, but **failed analytical coverage review**: one real call, zero repairs, 83,252 input / 3,689 output tokens, four answers and eight findings. It supplied 177 of 447 retained representations and requested no additional pages. It omitted already supplied surrounding process/module timelines, per-lead details, authentication and communications for the filename-only leads; one session relationship lacked its process endpoint citation. Full-query literal counts were correctly reported, with their scope. This isolated synthesis-only replay used `gpt-5.6-terra` / medium, preserved source/query arrays and frozen files, made no Splunk searches and did not connect to the application database. See `v26-replay/live-1/quality-review.json` and its claim/coverage evidence. It is not full-hunt or independent analyst acceptance.
- Completed and saved the older v21 four-call prototype review in `v21/focused-synthesis-live/quality-review.json`, preserving its previous pause status separately. It also fails coverage: native identifiers, surrounding timelines, filename-only lead correlations and scoped counts are missing; communications relationship citations omit the process endpoint. Its four calls consumed 308,952 input / 6,853 output tokens. More calls did not establish acceptance, and versions/context differ, so this is not a controlled causal comparison.
- Prompt **1.27** addresses two demonstrated handoff problems: prefer complete final context when it fits existing limits before reserving capacity for retrieval, and distinguish citation labels from native process/session facts requested in answers. Larger contexts still use bounded retrieval; a failed submitted final call is not restarted as retrieval. The new regression reproduced one of two retained records being unnecessarily omitted before the fix. All **704 backend tests passed, no skips**, with the same two dependency warnings; mypy passed on 55 source files. The 29-case production composition checks cover complete-context selection, oversized-context retrieval, submitted-call cutoff, checkpoint recovery, repeated pages and cancellation. Evidence is under `v27-full-context/`; analytical replay remains pending. No deployment has occurred.
