...
The local SPL policy 1.5 candidate checks field references in query text as well as the declared input fields. It rejects undiscovered source fields and validates calculated fields in definition order, while preserving supported quoted names and aggregate/rename aliases. Versioned function allowlists reject lookup access, searches embedded in strings and customer functions. Rejected queries are not submitted. Deploy API and worker together and approve a fresh plan when adopting this policy.

Execution results include per-attempt contract and grounding checks in `usage.model_output_checks`, alongside request characters, estimated input tokens and provider-reported input/output tokens. Missing historical or failed-transport usage remains unknown. The known-answer command's `synthetic` mode verifies scripted scorer fixtures only. Its `observed` mode scores actual result exports plus independent analyst judgments; instructions and metric definitions are in the operator runbook. Passing tests or successful repairs do not establish first-pass model quality.
